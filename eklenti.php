    ...
    if($eklenticek=$eklentisor->fetch(PDO::FETCH_ASSOC))
      {?>
        <div align="center" class="container">
          ...
          <img style="margin-bottom: 4%; border: 1px transparent; background: #edf6f9; padding: 10px; box-shadow: 0px 0px 15px #888888;align: center; height:400px;"src="<?php echo $eklenticek['eklenti_resim']?>">
          ...
          if($indirimcek=$indirimsor->fetch(PDO::FETCH_ASSOC))
          {
            $yenifiyat = ($eklenticek['eklenti_fiyat'] / 100) * (100 - $indirimcek['indirim_miktar']);?>
             <a href="https://pluginmerkezi.com/admin/sartlar"><button style="margin-top: 3%;"type="submit" class="btn btn-success">İndirimli: <?php echo $yenifiyat ?>TL <del><?php echo $eklenticek['eklenti_fiyat']."TL"?></del></button></a><?php 
          }
          else
          {
              ?>
              <a href="https://pluginmerkezi.com/admin/sartlar"><button style="margin-top: 3%;"type="submit" class="btn btn-success">Satın Al: <?php echo $eklenticek['eklenti_fiyat']?>TL</button></a><?php 
          }?>
            </div>
          <?php } 
          else
